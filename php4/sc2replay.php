...
class SC2Replay {
	var $gameSpeeds = array(0 => "Slower", 1=> "Slow", 2=> "Normal", 3=> "Fast", 4=> "Faster");
	var $gameSpeedCE = array(0 => 39, 1=> 44, 2=> 60, 3=> 64, 4=> 64); // estimates, weird values
	var $colorIndices = array(1 => "Red", 2=> "Blue", 3=> "Teal", 4=> "Purple", 5=> "Yellow", 6 => "Orange", 7=> "Green", 8=> "Pink");
	var $players; //array, indices: color, team, sname, lname, race, startRace, handicap, ptype
	var $gameLength; // game length in seconds
	var $mapName;
	var $gameSpeed; // game speed, number from 0-4. see $gameSpeeds array above
	var $teamSize; // team size in the format xvx, eg. 1v1
	var $gamePublic;
	var $version;
	var $build;
	var $events; // contains an array of the events in replay.game.events file
	var $debug; // debug, currently true or false
	var $debugNewline; // contents are appended to the end of all debug messages
	var $messages; // contains an array of the chat log messages
	function SC2Replay() {
	  $this->__construct();
	}
...
// sync event is 4 bytes, with a sync window of 1/8th to 1/16th of a second <---- UNCERTAIN, a theory
	...
	function debug($message) { echo $message.($this->debugNewline); }
	...
	function getGameSpeedText() { return $this->gameSpeeds[$this->gameSpeed]; }
	...
	function parseAttributesFile($string) {
			...
			switch ($attributeId) {
				...
				case 0x0BBA: // color index
					if ($this->build >= 15449) {
						...
						$this->players[$playerId]["sColor"] = $this->colorIndices[intval(substr($attribVal,2))];
					}
					else {
						$this->players[$playerId]["colorIndex"] = intval($attribVal);
						$this->players[$playerId]["sColor"] = $this->colorIndices[intval($attribVal)];
					}
					break;
				default:
			}
	...
	function parseGameEventsFile($string) {
		$numByte = 0;
		$len = strlen($string);
		$playerLeft = array();
		$events = array();
	...
	function parseTimeStamp($string, &$numByte) {
