...
class MPQFile {
	var $filename;
	var $fp;
	var $hashtable,$blocktable;
	var $hashTableSize, $blocKTableSize;
	var $headerOffset;
	var $init;
	var $verMajor;
	var $build;
	...
	var $gameLen;
	var $cryptTable;
	function MPQFile($filename, $autoparse = true, $debug = 0) {
	  $this->__construct($filename,$autoparse,$debug);
	}
	...
	function parseHeader() {
		...
		$hashTable = $this->decryptStuff($tmp,$this->hashStuff("(hash table)", MPQ_HASH_FILE_KEY));
		if ($this->debug) {
		...
		for ($i = 0;$i < $blockSize;$i++)
			$tmp[$i] = $this->readUInt32();
		$blockTable = $this->decryptStuff($tmp,$this->hashStuff("(block table)", MPQ_HASH_FILE_KEY));		
	...
	function readFile($filename) {
		...
		$hashA = $this->hashStuff($filename, MPQ_HASH_NAME_A);
		$hashB = $this->hashStuff($filename, MPQ_HASH_NAME_B);
		$hashStart = $this->hashStuff($filename, MPQ_HASH_TABLE_OFFSET) & ($this->hashTableSize - 1);
		$tmp = $hashStart;
		$blockSize = -1;
		do {
			if (($this->hashtable[$tmp*4 + 3] == MPQ_HASH_ENTRY_DELETED) || ($this->hashtable[$tmp*4 + 3] == MPQ_HASH_ENTRY_EMPTY)) return false;
		...
		for ($i = 0;$i < $c;$i++) {
			...
			if ($flag_compressed && (($flag_singleunit && ($blockSize < $fileSize)) || ($flag_checksums && ($sectorLen <  $this->sectorSize)))) {
				...
				switch ($compressionType) {
					case 2:
						$output .= $this->deflate_decompress($sectorData);
						break;
					default:
	...
	function hashStuff($string, $hashType) {
		...
		for ($i = 0;$i < $strLen;$i++) {
			...
			$seed1 = $this->cryptTable[($hashType << 8) + $next] ^ (uPlus($seed1,$seed2));
	...
	function decryptStuff($data, $key) {
		...
		$datalen = count($data);
		for($i = 0;$i < $datalen;$i++) {
			$seed = uPlus($seed,$this->cryptTable[0x400 + ($key & 0xFF)]);
